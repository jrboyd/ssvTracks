...
query_gr.l = reduce(promoters(tx_gr, 2e4, 2e4))
...
library(tidyverse)
chip_cfg = separate(chip_cfg, sample, into = c("cell", "mark"), sep = "_", remove = FALSE)
...
```{r}
track_gene_reference(gff_gr, query_gr = query_gr)
track_gene_reference(gff_gr, query_gr = query_gr, show_tss = TRUE)
```
...
rna_files = c(
  rna_files.all[grepl("MCF10A_RNA-Seq_R5", rna_files.all)],
  rna_files.all[grepl("MCF7_RNA-Seq_R1_GSM1944518", rna_files.all)],
  rna_files.all[grepl("DA-MB-231_RNA-Seq_R1_GSM1944521", rna_files.all)]
)
...
rna_cfg = rna_cfg %>% separate(., col = "sample", sep = "_", into = c("cell"), remove = FALSE)
rna_cfg$cell = rna_cfg$cell %>% sub("MDA-MB-231", "MDA231", .)
...
np_cfg$sample = np_cfg$file %>% 
  ...
  sub("_pooled.+", "", .)
np_cfg = np_cfg %>% 
  separate(., "sample", sep = "_", into = c("cell", "mark"), remove = FALSE)

track_features(np_grs, 
               query_gr = query_gr, 
...
```{r assembly1, fig.height=15, fig.width=8}
plots1 = list(
  track_chip(chip_cfg, query_gr = query_gr),
  
  track_rna.SE(rna_cfg, query_gr = query_gr, fill_VAR = "cell"),
  
  track_features(np_grs[grepl("K4ME3", names(np_grs))], 
                 query_gr = query_gr, 
                 sample_info_df = np_cfg, 

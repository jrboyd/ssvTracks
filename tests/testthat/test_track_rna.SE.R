...
test_that("track_rna.SE no spline", {
  p = track_rna.SE(bam_files, query_gr, fetch_fun = fetch_fun, nspline = 1)
  p
  testthat::expect_is(p, "ggplot")
})

test_that("track_rna.SE other strand, default spline", {
  p = track_rna.SE(bam_files, query_gr, fetch_fun = fetch_fun, target_strand = "-")
  p
  testthat::expect_is(p, "ggplot")
})

test_that("track_rna.SE low win", {
  p = track_rna.SE(bam_files, query_gr, fetch_fun = fetch_fun, nwin = 10, nspline = 5)
  p
  testthat::expect_is(p, "ggplot")
})

test_that("track_rna.SE max win (single bp)", {
  p = track_rna.SE(bam_files, resize(query_gr, 500), fetch_fun = fetch_fun, nwin = Inf, nspline = 1, x_scale = "bp")
  p
  testthat::expect_is(p, "ggplot")
})
...
test_that("track_rna.SE by cfg", {
  p = track_rna.SE(bw_cfg_dt, query_gr, fetch_fun = fetch_fun, nwin = 10)+
...
test_that("track_rna.SE by cfg - show_lines", {
  p = track_rna.SE(bw_cfg_dt, query_gr, fetch_fun = fetch_fun, nwin = 10, show_fill = FALSE, show_lines = TRUE, fill_VAR = "cell", color_VAR = "rep", color_mapping = c("R1" = "black"))+
    labs(y = "FE", title = "CTCF")
  p
  testthat::expect_is(p, "ggplot")
})
...
test_that("track_rna.SE by cfg - names_on_right", {
  p = track_rna.SE(bw_cfg_dt,
                resize(query_gr, 50*width(query_gr), fix = "center"),
                fetch_fun = fetch_fun,
                nwin = 10,

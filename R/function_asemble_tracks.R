#' assemble_tracks
#'
#' @param plot_list List. Contains ggplots and/or grobs. These should have have
...
#' @param query_gr GRanges. Most often the same query_gr used to generate
#'   tracks.  A smaller region to view in detail is also valid.
#' @param rel_heights Passed to rel_heights in cowplot::plot_grid. Controls
#'   relative heights of component plots.  Should be same length as plot_list.
#' @param final_ggplot_FUN_list list of functions to apply prior to width
...
#'
#' @return A combined ggplot of tracks stacked vertically with consistent
#'   x-axes.
#' @export
#'
#' @examples
...
#'
assemble_tracks = function(plot_list,
                           query_gr,
  ...
  suppressMessages({
    ...
    plot_list[[length(plot_list)]] = .apply_x_scale(plot_list[[length(plot_list)]], x_scale = x_scale, as.character(seqnames(query_gr)))
    if(!is.null(final_ggplot_FUN_list)){
      if(any(is.na(as.numeric(names(final_ggplot_FUN_list))))){
        stop("Invalid names for final_ggplot_FUN_list.  Must be able to convert to numeric.")
      }
      for(i in as.numeric(names(final_ggplot_FUN_list))){
        plot_list[[i]] = final_ggplot_FUN_list[[as.character(i)]](plot_list[[i]])
      }
    }
